...
public class Gui {
    ...
    private void update() {
        ...
        render(graphics, task, image.getWidth(), image.getHeight());
        graphics.dispose();
        panel.setImage(image);
    }

    private void render(Graphics2D graphics, RenderTask renderTask, int destinationWidth, int destinationHeight) {
        // we are flipping y
        Rectangle camera = renderTask.camera();
        AffineTransform transform = AffineTransform.getScaleInstance(
                (double) destinationWidth / camera.width(),
                (double) -destinationHeight / camera.height());
        transform.translate(-renderTask.camera().x(), renderTask.camera().y());
        graphics.setTransform(transform);
        graphics.setClip(
                renderTask.camera().x(),
                renderTask.camera().y() - renderTask.camera().height(),
...
//        }
        if (debug) {
            for (Rectangle actor : renderTask.actors()) {
                graphics.drawRect(actor.x(), actor.y(), actor.width(), actor.height());
            }
...
//            }
        ...
        if (debug) {
            graphics.drawString("fps: " + frameCount, 20, 20);
        }
    }
